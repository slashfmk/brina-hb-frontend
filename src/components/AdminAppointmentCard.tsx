...
const AdminAppointmentCard: React.FC<IAdminAppointment> = (props) => {
    ...
    return (
        <div className="flex flex-col w-full bg-pink-800 min-h-min p-4 mb-1 gap-5">

            <div className="flex sm:flex-row flex-col justify-between">
                <div className='sm:text-left text-center'>
                    <p className='text-slate-200'>Customer</p>
                    <div className='font-semibold text-slate-200'>{props.firstname} {props.lastname}</div>
                </div>

                <div>
                    <div className='text-slate-200'>Scheduled</div>
                    <div className='font-semibold text-slate-200'>{props.time}<br /> {props.date}</div>
                </div>

                <div>
                    <div className='text-slate-200'>Status</div>
                    <div className='font-semibold text-slate-200'>{props.complete ? 'Done' : 'incomplete'}</div>
                </div>

                <div>
                    <div className='text-slate-200'>Services</div>
                    <div className='font-semibold text-slate-200'>{props.services_count}</div>
                </div>

                <div className='sm:text-right text-center'>
                    <div className='text-slate-200'>Cost</div>
                    <div className='font-semibold text-slate-200'>${props.cost ?? 0}</div>
                </div>
            </div>
